...
import com.clube.sga.repository.UsuarioRepository;
...
public class UsuarioService implements UserDetailsService {
	...
	public Map<String, Object> buscarTodos(HttpServletRequest request) {
		...
		Page<Usuario> page = datatables.getSearch().isEmpty()
				? repository.findByPerfilNotAssociado(datatables.getPageable())/** repository.findAll(datatables.getPageable()) */
	...
	public void salvarCadastroPaciente(Usuario usuario) throws MessagingException {
		String crypt = new BCryptPasswordEncoder().encode(usuario.getSenha());
		usuario.setSenha(crypt);
		usuario.addPerfil(PerfilTipo.ASSOCIADO);
		repository.save(usuario);
	...
	public void ativarCadastroPaciente(String codigo) {
		...
		if (usuario.hasNotId()) {
			throw new AcessoNegadoException("Não foi possível etivar seu cadastro. "
